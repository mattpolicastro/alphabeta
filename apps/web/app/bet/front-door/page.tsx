...
function BetFrontDoorInner() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (cancelled) return;
      if (row) {
        const hydratedAb = asAbBet(row);
        setBet(hydratedAb);
        if (
          Object.values(hydratedAb).some(
            (v) => (v ?? "").toString().length > 0,
          )
        ) {
          // If the wager has any content, jump straight to stage 3.
          setStage(3);
        }
      }
      setHydrated(true);
  ...
  // Mirror the in-flight draft to Dexie on any change. Partial drafts persist
  // across sessions and screens; the lock is what makes it canonical.
  useEffect(() => {
    if (!hydrated || !id) return;
    if (Object.keys(bet).length === 0) return;
    void updateDraft(id, { articulation: toArticulation(bet) }).catch(() => {
      // Best-effort — if the row was locked between hydration and now,
      // updateDraft rejects and we leave the local state alone.
    });
  }, [bet, hydrated, id]);
