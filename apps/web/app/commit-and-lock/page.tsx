...
import { buildBetRecord, buildLockedSnapshot } from "@/lib/bet/factory";
import {
  readAbBet,
  readAbCommitted,
  writeAbCommitted,
  type AbBet,
  type AbCommitted,
} from "@/lib/bet/storage";
import { getDb } from "@/lib/db";
import { fingerprint } from "@/lib/integrity/fingerprint";

type LockState = "draft" | "confirming" | "locked";

export default function CommitAndLock() {
  const [bet, setBet] = useState<AbBet>({});
  const [state, setState] = useState<LockState>("draft");
  const [committed, setCommitted] = useState<AbCommitted | null>(null);
  const [error, setError] = useState<string | null>(null);

  useEffect(() => {
    setBet(readAbBet());
    const prior = readAbCommitted();
    if (prior) {
      setCommitted(prior);
      setState("locked");
    }
  ...
  const onLock = async () => {
    ...
    try {
      ...
      const record = buildBetRecord(bet, lockedAt, fp);
      await getDb().bets.add(record);
      const committedPayload: AbCommitted = {
        betId: record.id,
        lockedAt,
        fingerprint: fp,
      };
      writeAbCommitted(committedPayload);
  ...
  const onReset = () => {
    // Demo reset — clears the committed pointer locally, but does NOT delete
    // the locked Bet from IndexedDB (immutability). New locks create new rows.
    if (typeof window !== "undefined") {
      window.localStorage.removeItem("ab_committed");
    }
    setCommitted(null);
    setState("draft");
  };
  ...
  return (
    <div className="ab-wrap">
      ...
      <SpineRail steps={lifecycleSteps(isLocked)} />
          ...
          <div className="dashed-panel">
            ...
            {state === "locked" && (
              <>
                ...
                <Button onClick={onReset} style={{ fontSize: 10.5 }}>
                  ⟲ start a new draft (demo reset)
                </Button>
              </>
            )}

            {error && (
              <div className="mt-[10px] text-[11px] text-terra">
                {error}
              </div>
            )}
          </div>
...
function LockRow({
  ...
  return (
    <div className="flex gap-[10px] mb-[4px]">
      ...
      <span
        className={
          mono ? "font-mono text-[10.5px] break-all" : "text-[11.5px]"
        }
      >
        {v}
      </span>
...
function lifecycleSteps(commitDone: boolean): SpineStep[] {
  return [
    { n: 1, label: "bet", status: "done", href: "/" },
    {
      n: 2,
