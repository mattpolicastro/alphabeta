...
import { readAbCommitted, type AbBet } from "@/lib/bet/storage";
...
export default function Revisit() {
  ...
  useEffect(() => {
    let cancelled = false;
    (async () => {
      const committed = readAbCommitted();
      if (!committed) {
        if (!cancelled) setLoad({ kind: "missing" });
        return;
      }
      try {
        const bet = await getDb().bets.get(committed.betId);
        if (!bet) {
          if (!cancelled) setLoad({ kind: "missing" });
          return;
        }
...
function RevisitBody({
  ...
  const persist = async () => {
    try {
      await getDb().bets.update(bet.id, {
        status: "resolved",
