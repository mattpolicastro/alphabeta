...
export function probLiftAbove(c, t, lift) {
    if (lift <= -1)
        return 1;
...
export function expectedLoss(x, y) {
    const [lo, hi] = support(x.a, x.b);
    const meanY = y.a / (y.a + y.b);
    return integrate((v) => betaPdf(v, x.a, x.b) * (v * betainc(y.a, y.b, v) - meanY * betainc(y.a + 1, y.b, v)), lo, hi);
}
/** P(arm i has the highest rate) for independent Beta posteriors. */
export function probBest(arms, i) {
    const [lo, hi] = support(arms[i].a, arms[i].b);
    return integrate((x) => {
        let f = betaPdf(x, arms[i].a, arms[i].b);
        for (let j = 0; j < arms.length && f > 0; j++)
            if (j !== i)
                f *= betainc(arms[j].a, arms[j].b, x);
        return f;
    }, lo, hi);
}
/**
 ...
 */
export function liftInterval(c, t, level) {
    const q = (1 - level) / 2;
    let top = 1;
    while (probLiftAbove(c, t, top) > q && top < 1e6)
        top *= 2;
    const solve = (target) => {
        let lo = -1;
        let hi = top;
        for (let i = 0; i < 80 && hi - lo > 1e-10; i++) {
            const mid = (lo + hi) / 2;
            if (probLiftAbove(c, t, mid) > target)
                lo = mid;
            else
                hi = mid;
        }
...
export function bayes(input) {
    ...
    if (prior.a <= 0 || prior.b <= 0)
        throw new RangeError('prior a and b must be positive');
    assertFinite('threshold', threshold);
    if (threshold < 0 || threshold >= 1)
        throw new RangeError('threshold must be between 0 and 1 (0.015 for 1.5%)');
    assertFinite('level', level);
    if (level <= 0 || level >= 1)
        throw new RangeError('level must be strictly between 0 and 1');
    const posts = arms.map((arm) => ({ a: prior.a + arm.conversions, b: prior.b + arm.n - arm.conversions }));
    const describe = (arm, i) => {
        const p = posts[i];
        const q = (1 - level) / 2;
        return {
    ...
    const c = posts[0];
    const treatments = arms.slice(1).map((arm, k) => {
        const i = k + 1;
        const t = posts[i];
        ...
        let read = 'undecided';
        if (pAboveThreshold >= level)
            read = threshold > 0 ? 'beats-control-above-threshold' : 'beats-control';
        else if (pBeatsControl >= level)
            read = 'beats-control';
        else if (pNotWorse >= level)
            read = 'not-worse';
        else if (1 - pBeatsControl >= level)
            read = 'control-better';
        return {
            ...base,
