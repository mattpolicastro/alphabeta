...
- **`alphabeta.tools/lab/*` — standalone analysis tools (settled 2026-08-28; `sample-size`, `detectable-lift`, `srm` live 2026-09-04).** Stateless calculators on the *marketing* origin, a flat list **named by the practitioner's question** (method is a title qualifier, never the URL noun): `/lab/sample-size`, `/lab/detectable-lift`, `/lab/srm`, `/lab/pre-post`, `/lab/results`, `/lab/sequential`, `/lab/bayes`. Modeled on Kelly Wortham's `forwarddigital.org/tools` (five of these are ports of its R Shiny apps). Holdback and study are ceremony in the lock flow, not lab tools. Rules:
  ...
  - **Two CTAs, by what the tool is for (settled 2026-09-04).** Pre-registration tools — `sample-size`, `detectable-lift`, `sequential`'s plan — end in **"Lock as bet →"** (`app.alphabeta.tools/bet/new?from=…`). Evidence tools — `results`, `bayes`, `srm`, `sequential`'s checkpoints, `pre-post` — end in **"Attach to a bet →"** (`app.alphabeta.tools/bet/attach?from=…`): a result is evidence on a bet that already exists, never a new commitment. The accent mark goes on whichever CTA the page has; never both.
  - **Engines:** everything in the lab so far is closed-form TypeScript in `packages/analysis` (two-proportion z, Welch t, chi-square GoF, incomplete beta for Beta-Binomial, group-sequential boundaries ported from the R source). Pyodide is reserved for CausalImpact. Every function has an R and/or scipy oracle test; disagreements are characterized, never absorbed into tolerance.
...
- **No duplicated CSS or JS. Extract it.** Standing rule (2026-09-04): if the same rules, tokens, or script exist in two files, that is a bug to fix before the next visual change — not a style preference. `apps/landing` reached ten inline copies of the same base stylesheet and the token block had already drifted into two versions; a one-line optical fix cost ten edits. Same-origin static files on Pages have no reason to inline: one cached `<link>` is strictly better. Shared build artifacts (`analysis.js`, `special.js`) live at one path, not one copy per tool directory. The only exception is `lab/pre-post/`, whose page is built output from another repo and is overwritten on rebuild — and even it should link the shared sheet rather than carry its own.
