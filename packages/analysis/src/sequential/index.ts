...
export function qnormUpper(p: number): number {
  return -qnorm(p);
}
 ...
 */
export function uniroot(f: (x: number) => number, lower: number, upper: number, tol = 1.220703125e-4, maxit = 1000): number {
  let a = lower;
  let b = upper;
  let fa = f(a);
  let fb = f(b);
  if (!(fa * fb <= 0)) throw new RangeError('uniroot: f() values at end points not of opposite sign');
  let c = a;
  let fc = fa;
  if (fa === 0) return a;
  if (fb === 0) return b;
...
interface Grid { z: Float64Array; w: Float64Array; m: number }

/** gridpts.c: integration points (even indices) and Simpson midpoints (odd) between a and b, centred on mu. */
function gridpts(r: number, mu: number, a: number, b: number): Grid {
  ...
  if (ztem <= a) z[0] = a;
  else if (ztem >= b) { z[0] = b; done = true; }
  else z[0] = ztem;
  for (let i = 2; i < r6 && !done; i++) {
    if (i < r) ztem = mu - 3 - 4 * Math.log(r / i);
    else if (i <= r5) ztem = mu + 3 * (-1 + (i - r) / (2 * r));
    else ztem = mu + 3 + 4 * Math.log(r / (r6 - i));
    if (ztem > a) {
      j += 2;
      z[j] = ztem;
      if (ztem >= b) { z[j] = b; done = true; }
      z[j - 1] = (z[j] + z[j - 2]) / 2;
    }
  }
  if (j > 0) {
...
function h1(theta: number, g: Grid, I: number): Float64Array {
  const h = new Float64Array(g.m + 1);
  const mu = theta * Math.sqrt(I);
  for (let i = 0; i <= g.m; i++) h[i] = g.w[i] * dnorm(g.z[i] - mu);
  return h;
}

/** hupdate.c: propagate the weighted sub-density from one analysis to the next. */
function hupdate(theta: number, g2: Grid, g1: Grid, h1v: Float64Array, Ikm1: number, Ik: number): Float64Array {
...
export interface CrossingProbabilities { problo: number[]; probhi: number[] }

/** probrej.c (gsDesign::gsProbability for one theta): boundary-crossing probabilities per look. */
export function probRej(theta: number, I: number[], a: number[], b: number[], r = R_GRID): CrossingProbabilities {
  const k = I.length;
  const problo = new Array<number>(k);
  const probhi = new Array<number>(k);
 ...
 */
function gsBound(I: number[], problo: number[], probhi: number[], tol = 1e-6, r = R_GRID): { a: number[]; b: number[] } {
  const k = I.length;
  const a = new Array<number>(k);
  const b = new Array<number>(k);
  ...
  for (let i = 1; i < k; i++) {
    ...
    while ((adelta > tol || bdelta > tol) && j++ < EXTREMEZ) {
      let plo = 0; let phi = 0; let dplo = 0; let dphi = 0;
      atem = atem2;
      btem = btem2;
      for (let ii = 0; ii <= g.m; ii++) {
      ...
      if (atem2 > EXTREMEZ) atem2 = EXTREMEZ; else if (atem2 < -EXTREMEZ) atem2 = -EXTREMEZ;
      bdelta = probhi[i] - phi;
      if (bdelta < dphi) btem2 = btem + 1;
      else if (bdelta > -dphi) btem2 = btem - 1;
      else btem2 = btem + (probhi[i] - phi) / dphi;
      if (btem2 > EXTREMEZ) btem2 = EXTREMEZ; else if (btem2 < -EXTREMEZ) btem2 = -EXTREMEZ;
    ...
    a[i] = atem;
    b[i] = btem;
    if (adelta > tol || bdelta > tol) throw new RangeError(`gsBound: no convergence for boundary at look ${i + 1}`);
    if (i < k - 1) {
 ...
 */
function gsBound1(theta: number, I: number[], a: number[], probhi: number[], tol = 1e-6, r = R_GRID): { b: number[]; problo: number[] } {
  const k = I.length;
  const b = new Array<number>(k);
  const problo = new Array<number>(k);
  let rtIk = Math.sqrt(I[0]);
  ...
  for (let i = 1; i < k; i++) {
    ...
    b[i] = btem;
    problo[i] = plo;
    if (bdelta > tol) throw new RangeError(`gsBound1: no convergence for boundary at look ${i + 1}`);
    if (i < k - 1) {
      const g2 = gridpts(r, mu, a[i], b[i]);
      h = hupdate(theta, g2, g, h, I[i - 1], I[i]);
      g = g2;
  ...
  for (let i = 0; i < k; i++) if (b[i] - a[i] < 0) b[i] = a[i];
  return { b, problo };
}
...
function sfPowerIncrements(alpha: number, timing: number[], rho: number): number[] {
  const cum = timing.map((t) => alpha * Math.min(t, 1) ** rho);
  return cum.map((c, i) => c - (i === 0 ? 0 : cum[i - 1]));
}

export interface GsDesign {
  k: number;
  testType: 2 | 4;
  /** Per-arm information (visitors) at each look. */
  nI: number[];
  timing: number[];
  lower: number[];
  upper: number[];
  /** Standardised effect per unit of information: |z_α + z_β| / √n.fix. */
  delta: number;
  nFix: number;
  /** Per-look crossing probabilities under theta = 0 and theta = delta. */
  prob: { null: CrossingProbabilities; alt: CrossingProbabilities };
  /** Cumulative upper-crossing probability at delta — the design's power. */
  power: number;
  /** Cumulative upper-crossing probability at 0 — the alpha actually spent. */
  alphaSpent: number;
}

export interface GsDesignInput {
  k: number;
  testType: 2 | 4;
  /** One-sided alpha (the app passes (1 − conf) / tails). */
  alpha: number;
  beta: number;
  nFix: number;
  rhoUpper?: number;
  rhoLower?: number;
  /** Observed per-arm information per look; when given, `maxnIPlan` sets the timing. */
  nI?: number[];
  maxnIPlan?: number;
  tol?: number;
}
 ...
 */
export function gsDesign(input: GsDesignInput): GsDesign {
  ...
  if (!Number.isInteger(k) || k < 2) throw new RangeError('k must be an integer of at least 2');
  if (!(alpha > 0 && alpha < 1)) throw new RangeError('alpha must be strictly between 0 and 1');
  if (testType === 2 && alpha > 0.5) throw new RangeError('alpha must be at most 0.5 for a two-sided design');
  if (!(beta > 0 && beta < 1 - alpha)) throw new RangeError('beta must be strictly between 0 and 1 − alpha');
  if (!(nFix > 0)) throw new RangeError('n.fix must be positive');

  // gsDErrorCheck
  const delta = Math.abs(qnorm(alpha) + qnorm(beta)) / Math.sqrt(nFix);
  let timing: number[];
  let nIGiven: number[] | undefined;
  if (input.nI && input.nI.length) {
    nIGiven = input.nI.slice();
    if (nIGiven.length !== k) throw new RangeError('n.I must have one entry per look');
    for (let i = 0; i < k; i++) {
  ...
  if (testType === 2) {
    ...
    if (!nIGiven) {
      ...
      const noLower = new Array<number>(k).fill(-EXTREMEZ);
      const x0 = gsBound1(0, timing, noLower, falsepos, tol);
      const negB = x0.b.map((v) => -v);
      const betadiff1 = (Imax: number) => {
        const I = timing.map((t) => t * Imax);
      ...
      while (betadiff1(Ihigh) < 0) {
        if (Ihigh > 5 * nFix) throw new RangeError('Unable to derive sample size');
        Ilow = Ihigh;
        Ihigh *= 1.2;
      }
      // gsI1
      ...
      const b2 = x2.b.slice();
      if (-b2[k - 1] > x1.b[k - 1] - tol) b2[k - 1] = -x1.b[k - 1];
      nI = nIGiven;
      upper = x1.b;
      lower = b2.map((v) => -v);
    }
 ...
 */
export function testBinomial(x1: number, x2: number, n1: number, n2: number, delta0 = 0): number {
  const ntot = n1 + n2;
  const xtot = x1 + x2;
 ...
 */
export function ciBinomial(x1: number, x2: number, n1: number, n2: number, alpha: number): { lower: number; upper: number } {
  const delta = x1 / n1 - x2 / n2;
  const bp = (d: number, lowerTail: boolean) => {
  ...
  let upper: number;
  if (delta === -1) lower = -1;
  else if (testBinomial(x1, x2, n1, n2, -0.9999) < qnorm(alpha / 2)) lower = -1;
  else lower = uniroot((d) => bp(d, false), -0.9999, delta);
  if (delta === 1) upper = 1;
  else if (testBinomial(x1, x2, n1, n2, 0.9999) > -qnorm(alpha / 2)) upper = 1;
  else upper = uniroot((d) => bp(d, true), delta, 0.9999);
  return { lower, upper };
}
 ...
 * stays import-free).
 */
export function powerPropTest(p1: number, p2: number, sigLevel: number, power: number, tails: 1 | 2): number {
...
export interface SequentialPlanInput {
  /** Control conversion rate as a proportion in (0, 1). */
  baseline: number;
  /** Minimum detectable effect as a relative lift (0.10 = +10%). */
  mde: number;
  /** Non-inferiority margin as a relative proportion (0.02 = 2% below baseline). One-tailed only. Default 0. */
  margin?: number;
  /** Arms including control. Default 2. Above 2 the app adds Holm-adjusted efficacy bounds (see `holm`). */
  variants?: number;
  tails: 1 | 2;
  /** 1 − confidence. Two-sided total when tails = 2. */
  alpha: number;
  /** 1 − β. */
  power: number;
  /** Planned analyses including the final one, 2–20. */
  k: number;
  /** Daily visitors across all arms, for day estimates. Optional. */
  traffic?: number | null;
}
...
export interface PlannedLook {
  ...
  timing: number;
  /** Per-arm visitors at this look (gsDesign's n.I, unrounded). */
  perVariant: number;
  /** perVariant × 2 — the app's "Total Sample" (it sizes one comparison). */
  total: number;
  /** Futility (one-tailed) or lower efficacy (two-tailed) z bound. */
  lower: number;
  /** Efficacy z bound. */
  upper: number;
  /** Cumulative alpha spent through this look. */
  alphaSpent: number;
  /** Cumulative beta spent through this look (one-tailed: the futility spend). */
  betaSpent: number;
  /** Whole days to reach `total` at `traffic`/day; null without traffic. */
  days: number | null;
}

export interface SequentialPlan {
  fixed: { perVariant: number; total: number; nExact: number; days: number | null };
  looks: PlannedLook[];
  /** Per-arm and total visitors at the final look. */
  maxPerVariant: number;
  maxTotal: number;
  /** maxTotal / fixed total − 1. */
  increase: number;
  /** Achieved power at the MDE (cumulative efficacy crossing). */
  power: number;
  /** Alpha actually spent (cumulative efficacy crossing under H0; one-tailed: with the futility bound ignored). */
  alphaSpent: number;
  /** Expected per-arm n at delta = 0 and at the MDE. */
  expected: { null: number; alt: number };
  /**
   * When variants > 2: for each look, the Holm-adjusted efficacy bound by rank
   * (`[0]` applies to the largest |z| among the comparisons, and so on).
   */
  holm: number[][] | null;
  assumptions: {
    method: string;
...
function validatePlanInput(input: SequentialPlanInput) {
  ...
  if (baseline <= 0 || baseline >= 1) throw new RangeError('baseline must be strictly between 0 and 1');
  if (mde <= 0) throw new RangeError('mde must be positive');
  if (tails !== 1 && tails !== 2) throw new RangeError('tails must be 1 or 2');
  if (alpha <= 0 || alpha >= 1) throw new RangeError('alpha must be strictly between 0 and 1');
  if (power <= 0 || power >= 1) throw new RangeError('power must be strictly between 0 and 1');
  if (!Number.isInteger(k) || k < 2 || k > 20) throw new RangeError('k (number of looks) must be an integer from 2 to 20');
  if (margin < 0 || margin >= 1) throw new RangeError('margin must be at least 0 and below 1');
  if (margin > 0 && tails === 2) throw new RangeError('a non-inferiority margin applies to one-tailed tests only');
  if (!Number.isInteger(variants) || variants < 2) throw new RangeError('variants must be an integer of at least 2 (including control)');
  if (input.traffic != null && !(input.traffic > 0)) throw new RangeError('traffic must be positive when given');
  const treatmentRate = baseline * (1 + mde);
  if (treatmentRate >= 1) throw new RangeError('mde pushes the treatment rate to 100% or beyond');
  const alphaPerSide = alpha / tails;
  if (1 - power >= 1 - alphaPerSide) throw new RangeError('power must exceed the per-side alpha');
...
export function holmBounds(upper: number[], comparisons: number): number[][] {
  return upper.map((u) => {
    const p = pnormUpper(u);
    const row: number[] = [];
    for (let i = 1; i <= comparisons; i++) row.push(Math.abs(qnorm(p / (comparisons - i + 1))));
    return row;
  });
}

function planFromDesign(input: SequentialPlanInput, design: GsDesign, nFix: number, meta: ReturnType<typeof validatePlanInput>): SequentialPlan {
  const traffic = input.traffic ?? null;
  const days = (total: number) => (traffic ? Math.ceil(total / traffic) : null);
  const alphaCum: number[] = [];
  const betaCum: number[] = [];
  let ac = 0;
  let bc = 0;
  ...
  const looks: PlannedLook[] = design.nI.map((n, i) => ({
    index: i + 1,
  ...
  const en = (p: CrossingProbabilities) => {
    let s = 0;
    let stopped = 0;
 ...
 */
export function sequentialPlan(input: SequentialPlanInput): SequentialPlan {
...
export interface Checkpoint {
  /** Which planned look this data belongs to, 1..k. */
  index: number;
  control: { visitors: number; conversions: number };
  treatment: { visitors: number; conversions: number };
}

export type Boundary = 'upper' | 'lower' | 'middle';
export type Outcome = 'reject' | 'continue' | 'option' | 'inconclusive';

export interface AnalyzedCheckpoint extends Checkpoint {
  /** Per-arm n the app records for this look: (control + treatment visitors) / 2. */
  perVariant: number;
  controlRate: number;
  treatmentRate: number;
  /** treatment / control − 1. */
  lift: number;
  /** −testBinomial(control, treatment, delta0 = controlRate × margin): positive favours treatment. */
  z: number;
  lower: number;
  upper: number;
  boundary: Boundary;
  /** The app's status key, e.g. `early_upper`, `complete_lower_1tail`. */
  status: string;
  outcome: Outcome;
  /** Fraction of the planned maximum total sample reached. */
  fractionOfPlan: number;
}

export interface SequentialAnalysis {
  /** The design re-timed to the observed looks (unchanged when there are none). */
  plan: SequentialPlan;
  /** Per-arm n at every look after re-timing — the app's revised_n. */
  revisedPerVariant: number[];
  checkpoints: AnalyzedCheckpoint[];
  verdict: {
    /** The deciding checkpoint: the first that rejects, else the last evaluated. */
    checkpoint: number;
    outcome: Outcome;
    status: string;
    /** Adjusted (sequential) p-value and confidence, only on rejection. */
    pValue: number | null;
    confidence: number | null;
    /** Interval on the relative lift (treatment / control − 1), adjusted to the efficacy bound at that look. */
 ...
 */
export function reviseLooks(k: number, plannedMax: number, entered: { index: number; perVariant: number }[]): number[] {
  const anchors = new Map<number, number>();
  anchors.set(0, 0);
  for (const e of entered) anchors.set(e.index, e.perVariant);
  if (!anchors.has(k)) anchors.set(k, plannedMax);
  const idx = [...anchors.keys()].sort((a, b) => a - b);
  const out: number[] = [];
  for (let i = 1; i <= k; i++) {
    if (anchors.has(i)) { out.push(Math.round(anchors.get(i)!)); continue; }
    let lo = 0;
    let hi = k;
    for (const j of idx) { if (j < i) lo = j; if (j > i) { hi = j; break; } }
 ...
 */
export function sequentialAnalyze(input: SequentialPlanInput, checkpoints: Checkpoint[]): SequentialAnalysis {
  const meta = validatePlanInput(input);
  ...
  let prevN = 0;
  for (const c of cps) {
    if (!Number.isInteger(c.index) || c.index < 1 || c.index > k) throw new RangeError(`checkpoint index must be from 1 to ${k}`);
    if (seen.has(c.index)) throw new RangeError(`checkpoint ${c.index} is entered twice`);
    seen.add(c.index);
    for (const [arm, d] of [['control', c.control], ['treatment', c.treatment]] as const) {
      assertFinite(`${arm} visitors`, d.visitors);
      assertFinite(`${arm} conversions`, d.conversions);
      if (!Number.isInteger(d.visitors) || d.visitors < 1) throw new RangeError(`checkpoint ${c.index}: ${arm} visitors must be a positive integer`);
    ...
    if (n <= prevN) throw new RangeError(`checkpoint ${c.index}: total visitors must exceed the earlier checkpoint's`);
    prevN = n;
  }
  ...
  const analyzed: AnalyzedCheckpoint[] = [];
  let deciding: AnalyzedCheckpoint | null = null;
  for (const c of cps) {
    const i = c.index - 1;
    ...
    analyzed.push(row);
    if (!deciding || deciding.outcome !== 'reject') deciding = row;
    if (row.outcome === 'reject') break;
  }
