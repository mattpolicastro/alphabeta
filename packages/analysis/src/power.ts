...
export interface SampleSizeInput {
  /** Control conversion rate as a proportion in (0, 1). 2% is 0.02. */
  baseline: number;
  /**
   * Minimum detectable effect. With `mdeKind: 'relative'` it is a lift over
   * baseline as a proportion (0.10 = +10%, so 2% becomes 2.2%). With
   * `'absolute'` it is a difference in rate as a proportion (0.002 = +0.2
   * percentage points, so 2% becomes 2.2%). Must be non-zero; sign is the
   * direction of the hypothesised change.
   */
  mde: number;
  mdeKind: 'relative' | 'absolute';
  /** Number of arms including control. 2 is a plain A/B. */
  variants: number;
  /** 1 for a directional test (B better than A), 2 for a difference in either direction. */
  tails: 1 | 2;
...
export interface SampleSizeAssumptions {
  test: string;
  /** Treatment rate the test is sized to detect. */
  treatmentRate: number;
  /** MDE restated in the other unit. */
  absoluteMde: number;
  relativeMde: number;
  /** Comparisons the alpha is spread across: variants − 1, each arm vs control. */
  comparisons: number;
  correction: 'none' | 'bonferroni';
  /** alpha / comparisons — what each individual comparison is tested at. */
  alphaPerComparison: number;
  zAlpha: number;
  zBeta: number;
  /** Unrounded per-variant n before ceiling. */
  nExact: number;
}

export interface SampleSizeResult {
  /** Visitors required in each arm (equal allocation). */
  perVariant: number;
  /** perVariant × variants. */
  total: number;
...
export interface RuntimeInput {
  /** Total visitors required across all arms. */
  total: number;
...
function assertOpenUnit(name: string, v: number): void {
  if (v <= 0 || v >= 1) throw new RangeError(`${name} must be strictly between 0 and 1`);
}
...
export function sampleSize(input: SampleSizeInput): SampleSizeResult {
  ...
  if (mde === 0) throw new RangeError('mde must be non-zero');
